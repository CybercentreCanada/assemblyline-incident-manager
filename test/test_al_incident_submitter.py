...
class TestALIncidentSubmitter:
    ...
    def test_main(case, command_line_options, mocker):
        ...
        mocker.patch(
            "assemblyline_incident_manager.helper.get_client", return_value="blah"
        )
        mocker.patch(
            "assemblyline_incident_manager.al_incident_submitter._test_ingest_file"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                (1, "TLP:W", ["blah1", "blah2"], True, 100),
                {
                    "ttl": 1,
                    "classification": "TLP:W",
                    "services": {
                        "selected": ["blah1", "blah2"],
            ...
            (
                (1, "TLP:W", ["blah1"], False, 1000),
                {
                    "ttl": 1,
                    "classification": "TLP:W",
                    "services": {"selected": ["blah1"], "resubmit": []},
                    "priority": 1000,
                },
            ),
        ],
    ...
    def test_file_has_valid_size(mocker):
        ...
        assert _file_has_valid_size(too_small_file_path, too_small_file_path) == (
            False,
            0,
            1,
        )
        ...
        assert _file_has_valid_size(too_big_file_path, too_big_file_path) == (
            False,
            1,
            0,
        )
        ...
        mocker.patch(
            "os.path.getsize", return_value=(MAX_FILE_SIZE + MIN_FILE_SIZE) / 2
        )
        assert _file_has_valid_size(just_right_file_path, just_right_file_path) == (
            True,
            0,
            0,
        )
    ...
    def test_ingest_file(dummy_al_client_class_instance):
        ...
        _ingest_file(
            "blah", "blah", "blah", dummy_al_client_class_instance, {}, "blah", True
        )
        assert True
    ...
    def test_get_most_recent_file_path(mocker):
        ...
        from assemblyline_incident_manager.al_incident_submitter import (
            _get_most_recent_file_path,
            FILE_PATHS,
        )

        mocker.patch(
            "assemblyline_incident_manager.al_incident_submitter.hash_table",
            return_value=[],
        )
        if path.exists(FILE_PATHS):
            remove(FILE_PATHS)
